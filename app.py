...
def init_database():
    ...
    # Tabla de ventanas de CARGA DE NOTAS (apertura / cierre) por ámbito:
    #  ambito: 'ESTADO' | 'MUNICIPIO' | 'AULA_TALLER' | 'PROGRAMA' | 'GLOBAL'
    #  estado_ventana: 'ABIERTA' | 'CERRADA'
    c.execute("""CREATE TABLE IF NOT EXISTS ventanas_notas (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ambito TEXT NOT NULL,
        valor TEXT NOT NULL DEFAULT '',
        estado_ventana TEXT DEFAULT 'ABIERTA',
        periodo TEXT DEFAULT '',
...
_MALLAS_PNFA_ESP = {
    ...
    # --- Especialización derivada del formato oficial afrovenezolano (T1-3).
    #     Cargada en MAYÚSCULAS. Coincide con los T1-3 del magíster afro.
    'ESPECIALISTA EN EDUCACIÓN Y PEDAGOGÍAS AFROVENEZOLANAS': [
        (1, 'PRIMER TRAYECTO', 'RUTA MIGRATORIA Y DIÁSPORA AFRICANA', 3),
        (1, 'PRIMER TRAYECTO', 'TERRITORIO, TERRITORIALIDAD Y AFRODESCENDENCIA', 3),
        (1, 'PRIMER TRAYECTO', 'PEDAGOGÍA CIMARRONA', 3),
        (2, 'SEGUNDO TRAYECTO', 'RESISTENCIA CULTURAL', 3),
        (2, 'SEGUNDO TRAYECTO', 'AFROEPISTEMOLOGÍA', 3),
        (2, 'SEGUNDO TRAYECTO', 'EDUCACIÓN AFROVENEZOLANA Y SABERES ANCESTRALES PARA LA SOSTENIBILIDAD', 3),
        (3, 'TERCER TRAYECTO', 'LA ESPIRITUALIDAD DEL CUMBE Y LA PEDAGOGÍA ANCESTRAL', 3),
        (3, 'TERCER TRAYECTO', 'REDES CIRCUITALES Y REGIONALES', 3),
        (3, 'TERCER TRAYECTO', 'POLÍTICA EDUCATIVA Y AFRODESCENDIENTES', 3),
    ],
}
...
_MALLAS_MAESTRIA = {
    ...
    # ============================================================
    # MALLAS OFICIALES CARGADAS EN MAYÚSCULAS (formatos REVISADOS).
    # TÍTULOS OFICIALES respetados (MAESTRÍA ≠ MAGÍSTER).
    # ============================================================
    'MAESTRÍA EN CIENCIAS NATURALES PARA EDUCACIÓN MEDIA': [
        (1, 'PRIMER TRAYECTO', 'PEDAGOGÍA DEL AMOR, DEL EJEMPLO Y DE LA CURIOSIDAD', 3),
        (1, 'PRIMER TRAYECTO', 'SEMINARIO I: INVESTIGAR PARA TRANSFORMAR', 3),
        ...
        (3, 'TERCER TRAYECTO', 'SEMINARIO III: INVESTIGACIÓN PARA LA TRANSFORMACIÓN DE LA PRÁCTICA', 3),
        (3, 'TERCER TRAYECTO', 'EDUCACIÓN SOCIO COMUNITARIA EN EL CONTEXTO DE LAS CIENCIAS, LA TECNOLOGÍA Y LA PRODUCCIÓN', 3),
        (4, 'CUARTO TRAYECTO', 'FILOSOFÍA ROBINSONIANA', 3),
        (4, 'CUARTO TRAYECTO', 'SISTEMATIZACIÓN DE EXPERIENCIAS COMO METODOLOGÍA', 3),
        (4, 'CUARTO TRAYECTO', 'PEDAGÓGICOS-DIDÁCTICOS DE LA EDUCACIÓN EN CIENCIAS', 3),
    ],
    'MAESTRÍA EN GEOGRAFÍA, HISTORIA Y CIUDADANÍA PARA EDUCACIÓN MEDIA': [
        (1, 'PRIMER TRAYECTO', 'PEDAGOGÍA DEL AMOR, DEL EJEMPLO Y DE LA CURIOSIDAD', 3),
        (1, 'PRIMER TRAYECTO', 'SEMINARIO I: ENFOQUE SOCIO CRÍTICO PARA LA TRANSFORMACIÓN DE LA PRÁCTICA', 3),
        ...
        (2, 'SEGUNDO TRAYECTO', 'DESCOLONIZACIÓN DEL PENSAMIENTO ACCIÓN ESTRATÉGICA PARA FORTALECER EL TEJIDO IDENTITARIO NUESTRO', 3),
        (3, 'TERCER TRAYECTO', 'ESTUDIANTE ESCUELA FAMILIA Y COMUNIDAD', 3),
        (3, 'TERCER TRAYECTO', 'SEMINARIO III: INVESTIGACIÓN PARA LA TRANSFORMACIÓN DE LA PRÁCTICA III: PROPUESTA PEDAGÓGICA EN GHC', 3),
        (3, 'TERCER TRAYECTO', 'LA UNIÓN NUESTRA AMERICANA EN EL SIGLO XX UNA REINTERPRETACIÓN DESDE LA PERSPECTIVA DE MIRANDA, BOLÍVAR', 3),
        (4, 'CUARTO TRAYECTO', 'SEMINARIO DE INVESTIGACIÓN I', 3),
        (4, 'CUARTO TRAYECTO', 'SEMINARIO DE INVESTIGACIÓN II', 3),
        (4, 'CUARTO TRAYECTO', 'SEMINARIO DE INVESTIGACIÓN III', 3),
    ],
    # MLEN: el formato oficial SÓLO trae T1-T3 (el T4 aparece como 'unidad
    # curricular' sin definir). Se cargan las unidades existentes; el CUARTO
    # TRAYECTO queda PENDIENTE (no se inventan materias).
    'MAESTRÍA EN LENGUA Y COMUNICACIÓN PARA EDUCACIÓN MEDIA': [
        (1, 'PRIMER TRAYECTO', 'PEDAGOGÍA DEL AMOR, DEL EJEMPLO Y DE LA CURIOSIDAD', 3),
        (1, 'PRIMER TRAYECTO', 'SEMINARIO I ENFOQUE SOCIO CRÍTICO PARA LA TRANSFORMACIÓN DE LA PRÁCTICA', 3),
        (1, 'PRIMER TRAYECTO', 'PRÁCTICAS SOCIALES DE LA LECTURA Y LA ESCRITURA', 3),
        (2, 'SEGUNDO TRAYECTO', 'CLIMA ESCOLAR CULTURA EMANCIPADORA PARA EL VIVIR BIEN', 3),
        (2, 'SEGUNDO TRAYECTO', 'SEMINARIO II: FORMACIÓN E INVESTIGACIÓN SEGÚN EL CONTEXTO PROBLEMÁTICO Y LÍNEA DE INVESTIGACIÓN', 3),
        (2, 'SEGUNDO TRAYECTO', 'COMUNICACIÓN: LENGUAJE Y PODER', 3),
        (3, 'TERCER TRAYECTO', 'ESTUDIANTE ESCUELA FAMILIA Y COMUNIDAD', 3),
        (3, 'TERCER TRAYECTO', 'SEMINARIO DE INVESTIGACIÓN III', 3),
        (3, 'TERCER TRAYECTO', 'NARRATIVA ORAL. ESCRITA Y AUDIOVISUAL DE VENEZUELA', 3),
    ...
    'MAGÍSTER EN EDUCACIÓN INDÍGENA': [
        ...
        (3, 'TERCER TRAYECTO', 'SEMINARIO III: MAESTRAS Y MAESTROS QUE CONOZCAN Y SE APROPIAN DE LA REALIDAD INDÍGENA', 3),
        (3, 'TERCER TRAYECTO', 'LOS PUEBLOS INDÍGENAS DESDE SUS SABERES ANCESTRALES: ARTE, GASTRONOMÍA, SOCIO PRODUCTIVO', 3),
        (4, 'CUARTO TRAYECTO', 'FILOSOFÍA DE LA EDUCACIÓN INDÍGENA', 3),
        (4, 'CUARTO TRAYECTO', 'METODOLOGÍA DE EDUCACIÓN PROPIA INDÍGENA', 3),
        (4, 'CUARTO TRAYECTO', 'MÉTODOS DE INVESTIGACIÓN CUALITATIVO EN EL CONTEXTO INDÍGENA', 3),
    ],
    'MAGÍSTER EN EDUCACIÓN EN FRONTERA': [
        (1, 'PRIMER TRAYECTO', 'COLECTIVOS DE RESISTENCIA E INSURGENCIA DE LA EDUCACIÓN EN FRONTERAS', 3),
        (1, 'PRIMER TRAYECTO', 'SEMINARIO I: ENFOQUE CRÍTICO Y GEOHISTÓRICO EN LAS PRÁCTICAS SOCIOEDUCATIVAS DE LA FRONTERA', 3),
        (1, 'PRIMER TRAYECTO', 'COMPORTAMIENTO DE LA FRONTERA VIVA', 3),
        (2, 'SEGUNDO TRAYECTO', 'CLIMA ESCOLAR CULTURA EMANCIPADORA PARA EL VIVIR BIEN EN FRONTERAS: EDUCANDO PARA LA PAZ', 3),
        (2, 'SEGUNDO TRAYECTO', 'SEMINARIO II: INVESTIGACIÓN ACCIÓN: CONTEXTO-EDUCATIVO EN LA FRONTERA', 3),
        (2, 'SEGUNDO TRAYECTO', 'GEOPOLÍTICA INTERNACIONAL: RELACIONES INTERNACIONALES TRATADOS Y ACUERDOS', 3),
        (3, 'TERCER TRAYECTO', 'VISIÓN INTER Y TRAS DISCIPLINAR EN FRONTERAS', 3),
        (3, 'TERCER TRAYECTO', 'SEMINARIO III: ESTRATEGIAS TRANSFORMADORAS Y SISTEMATIZACIÓN PEDAGÓGICA EN EL MARCO DEL SABER Y TRABAJO', 3),
        (3, 'TERCER TRAYECTO', 'NUEVAS SUBJETIVIDADES EN LA EDUCACIÓN EN FRONTERAS', 3),
        (4, 'CUARTO TRAYECTO', 'PRÁCTICA DE COMUNICACIÓN MEDIÁTICA EN CONTEXTOS DE GUERRA DE CUARTA GENERACIÓN', 3),
        (4, 'CUARTO TRAYECTO', 'SEMINARIO IV: EPISTEMOLOGÍA SOCIOCRÍTICA: INVESTIGAR PARA TRANSFORMAR', 3),
        (4, 'CUARTO TRAYECTO', 'ELEMENTOS TRANSFORMADORES DE LA CULTURA FRONTERIZA', 3),
    ],
    'MAGÍSTER EN EDUCACIÓN Y PEDAGOGÍAS AFROVENEZOLANAS': [
        (1, 'PRIMER TRAYECTO', 'RUTA MIGRATORIA Y DIÁSPORA AFRICANA', 3),
        (1, 'PRIMER TRAYECTO', 'TERRITORIO, TERRITORIALIDAD Y AFRODESCENDENCIA', 3),
        ...
        (3, 'TERCER TRAYECTO', 'REDES CIRCUITALES Y REGIONALES', 3),
        (3, 'TERCER TRAYECTO', 'POLÍTICA EDUCATIVA Y AFRODESCENDIENTES', 3),
        (4, 'CUARTO TRAYECTO', 'IDENTIDAD PEDAGÓGICA AFRODESCENDIENTE', 3),
        (4, 'CUARTO TRAYECTO', 'SEMINARIO DE INVESTIGACIÓN: DE LOS SABERES CIMARRONES A LA CORRIENTE EDUCATIVA AFROVENEZOLANA', 3),
        (4, 'CUARTO TRAYECTO', 'INVESTIGACIÓN FAMILIA AFROVENEZOLANA Y AUTO RECONOCIMIENTO', 3),
    ],
}
...
def _parsear_datos_cedula(texto):
    ...
    primer_apellido, segundo_apellido, sexo (M/F). Todo queda editable.
    """
    out = {}
    if not texto:
        return out
    ...
    # están impresos en MAYÚSCULAS. Las etiquetas ("APELLIDOS"/"NOMBRES") suelen
    # salir muy distorsionadas por el OCR, así que en vez de buscarlas se detectan
    # las líneas que "parecen un nombre": secuencias de palabras (2 o más letras),
    # descartando el texto fijo de la tarjeta.
    _STOP = {
        "REPUBLICA", "REPÚBLICA", "BOLIVARIANA", "DE", "DEL", "LA", "EL",
        "VENEZUELA", "CEDULA", "CÉDULA", "IDENTIDAD", "VENEZOLANO",
        "VENEZOLANA", "EXTRANJERO", "EXTRANJERA", "SOLTERO", "SOLTERA",
        "CASADO", "CASADA", "DIVORCIADO", "DIVORCIADA", "VIUDO", "VIUDA",
        "NOMBRES", "NOMBRE", "APELLIDOS", "APELLIDO", "SEXO", "FECHA",
        "NACIMIENTO", "ESTADO", "CIVIL", "DIRECTOR", "DIRECTORA", "FIRMA",
        "REGISTRO", "ELECTORAL", "SAIME", "SERVICIO", "ADMINISTRATIVO",
        "MIGRACION", "MIGRACIÓN", "IDENTIFICACION", "IDENTIFICACIÓN",
        "NACIONAL", "MASCULINO", "FEMENINO", "CIMV", "P",
    }

    def _buscar_runs(lineas, patron):
        """Devuelve, por cada línea, la secuencia más larga de palabras que casan
        con 'patron' y no son palabras fijas de la cédula (en MAYÚSCULAS)."""
        cand = []
        for l in lineas:
            palabras = re.split(r"[^A-Za-zÁÉÍÓÚÑÜáéíóúñü]+", l)
            run, mejor = [], []
            for w in palabras:
                if patron.match(w) and w.upper() not in _STOP:
                    run.append(w.upper())
                else:
    ...
    lineas = txt.split("\n")
    # 1º intento: palabras TODO EN MAYÚSCULAS (como aparece el titular).
    _caps = re.compile(r"^[A-ZÁÉÍÓÚÑÜ]{2,}$")
    candidatos = _buscar_runs(lineas, _caps)
    # 2º intento (respaldo): si no se hallaron 2 líneas, aceptar también nombres
    # en formato "Título" (Primera Letra Mayúscula), para documentos donde el OCR
    # no devolvió mayúsculas. Se conservan primero los de mayúsculas.
    if len(candidatos) < 2:
        _title = re.compile(r"^[A-ZÁÉÍÓÚÑÜ][a-záéíóúñü]{1,}$")
        for e in _buscar_runs(lineas, _title):
            if e not in candidatos:
                candidatos.append(e)
    # Primer candidato = APELLIDOS, segundo = NOMBRES (orden impreso en la cédula).
    if len(candidatos) >= 1:
        ap = candidatos[0]
        out["primer_apellido"] = ap[0]
        if len(ap) > 1:
            out["segundo_apellido"] = ap[1]
    if len(candidatos) >= 2:
        no = candidatos[1]
        out["primer_nombre"] = no[0]
        if len(no) > 1:
            out["segundo_nombre"] = no[1]
    return out
...
# ------------------------------------------------------------
# ALINEACIÓN de las mallas sembradas al catálogo OFICIAL.
# Las mallas oficiales se cargaron con nombres antiguos (o con pequeñas
# variantes de redacción) que NO coinciden con la denominación oficial del
# catálogo (54 programas). Este mapa nombre_sembrado -> nombre_oficial permite
# que cada malla quede pegada al programa correcto y aparezca en su lugar.
# ------------------------------------------------------------
_REMAP_MALLAS_OFICIAL = {
    # --- PNF (recorrido bachiller: nombres antiguos -> oficial) ---
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN MATEMÁTICA": "PROFESORA O PROFESOR DE EDUCACIÓN EN EL ÁREA DE MATEMÁTICA",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN FÍSICA": "PROFESORA O PROFESOR EN EL ÁREA DE FÍSICA",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN QUÍMICA": "PROFESORA O PROFESOR EN EL ÁREA DE QUÍMICA",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN BIOLOGÍA": "PROFESORA O PROFESOR DE EDUCACIÓN EN EL ÁREA DE BIOLOGÍA",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN GEOGRAFÍA, HISTORIA Y CIUDADANÍA": "LICENCIADA O LICENCIADO EN EDUCACIÓN EN GEOGRAFÍA, HISTORIA Y CIUDADANÍA",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN EDUCACIÓN FÍSICA": "LICENCIADA O LICENCIADO EN EDUCACIÓN FÍSICA",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN INGLÉS": "LICENCIADA O LICENCIADO EN EDUCACIÓN EN LENGUAS EXTRANJERAS MENCIÓN INGLÉS",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN EDUCACIÓN INICIAL": "LICENCIADA O LICENCIADO EN EDUCACIÓN INICIAL",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN LENGUA": "LICENCIADA O LICENCIADO EN EDUCACIÓN MEDIA EN LENGUA",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN EDUCACIÓN PRIMARIA": "LICENCIADA O LICENCIADO EN EDUCACIÓN PRIMARIA",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN ADMINISTRACIÓN Y GESTIÓN ESCOLAR": "LICENCIADA O LICENCIADO EN GESTIÓN INSTITUCIONAL MENCIÓN: ADMINISTRACIÓN Y GESTIÓN ESCOLAR",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN GESTIÓN Y MANTENIMIENTO DEL AMBIENTE ESCOLAR": "LICENCIADA O LICENCIADO EN GESTIÓN INSTITUCIONAL MENCIÓN: MANTENIMIENTO DE AMBIENTES ESCOLARES",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN EDUCACIÓN DE JÓVENES, ADULTOS Y ADULTAS": "LICENCIADA O LICENCIADO EN EDUCACIÓN DE JOVENES ADULTAS Y ADULTOS",
    "LICENCIADO/A EN EDUCACIÓN, MENCIÓN EDUCACIÓN ESPECIAL": "LICENCIADA O LICENCIADO EN EDUCACIÓN ESPECIAL",
    # --- PNFA-E (Especializaciones: variantes -> oficial) ---
    "ESPECIALIZACIÓN EN EDUCACIÓN INDÍGENA": "ESPECIALISTA EN EDUCACIÓN INDÍGENA",
    "ESPECIALIZACIÓN EN EDUCACIÓN EN FRONTERAS": "ESPECIALISTA EN EDUCACIÓN EN FRONTERA",
    "ESPECIALIZACIÓN EN EDUCACIÓN DE LA SEXUALIDAD": "ESPECIALISTA EN EDUCACIÓN INTEGRAL DE LA SEXUALIDAD",
    "ESPECIALIZACIÓN EN EDUCACIÓN DE JÓVENES, ADULTOS Y ADULTAS": "ESPECIALISTA EN EDUCACIÓN DE JOVENES, ADULTAS Y ADULTOS",
    "ESPECIALIZACIÓN EN MATEMÁTICA": "ESPECIALIZACIÓN EDUCACIÓN MATEMÁTICA",
    "ESPECIALIZACIÓN EN GEOGRAFÍA, HISTORIA Y CIUDADANÍA": "ESPECIALIZACIÓN EDUCACIÓN EN GEOGRAFÍA, HISTORIA Y CIUDADANÍA",
    "ESPECIALIZACIÓN EN EDUCACIÓN Y TECNOLOGÍA DE LA INFORMACIÓN Y COMUNICACIÓN": "ESPECIALIZACIÓN EDUCACIÓN Y TECNOLOGÍA DE LA INFORMACIÓN Y COMUNICACIÓN",
    "ESPECIALIZACIÓN EN DERECHO DE NIÑOS, NIÑAS Y ADOLESCENTES, CONVIVENCIA SOLIDARIA Y PAZ": "ESPECIALIZACIÓN EN DERECHOS DE LOS NIÑOS, NIÑAS Y ADOLESCENTES CONVIVENCIA SOLIDARIA Y PAZ",
    "ESPECIALIZACIÓN EN EDUCACIÓN EN AGROECOLOGÍA": "ESPECIALIZACIÓN EDUCACIÓN EN AGROECOLOGÍA",
    "ESPECIALIZACIÓN EN LENGUA Y COMUNICACIÓN": "ESPECIALIZACIÓN EDUCACIÓN EN LENGUA Y COMUNICACIÓN",
    "ESPECIALIZACIÓN EN LENGUA EXTRANJERA INGLÉS PARA PRIMARIA": "ESPECIALIZACIÓN EN LENGUAS EXTRANJERAS INGLÉS PARA EDUCACIÓN PRIMARIA",
    "ESPECIALIZACIÓN EN LENGUA EXTRANJERA: INGLÉS": "ESPECIALIZACIÓN EN LENGUAS EXTRANJERAS: INGLÉS",
    "ESPECIALIZACIÓN EN PEDAGOGÍA CULTURAL E INTERCULTURALIDAD": "ESPECIALISTA EN PEDAGOGÍA CULTURAL E INTERCULTURALIDAD",
    "ESPECIALIZACIÓN EN EDUCACIÓN MEDIA TÉCNICA Y PROFESIONAL": "ESPECIALISTA EN EDUCACIÓN MEDIA TÉCNICA Y PROFESIONAL",
    # --- PNFA-M (Maestrías / Magíster: variantes -> oficial) ---
    "MAESTRÍA EN PEDAGOGÍA CULTURAL E INTERCULTURALIDAD": "MAGÍSTER EN PEDAGOGÍA CULTURAL E INTERCULTURALIDAD",
    "MAESTRÍA EN MATEMÁTICA PARA EDUCACIÓN MEDIA": "MAESTRÍA EN MATEMÁTICAS PARA EDUCACIÓN MEDIA",
}
...
def _seed_mallas_oficiales(c):
    ...
    # (5) ALINEACIÓN FINAL al catálogo oficial: las mallas sembradas con nombre
    #     antiguo o con variante de redacción se re-pegan al nombre OFICIAL del
    #     programa, para que aparezcan en su lugar dentro de "Mallas
    #     Curriculares". Si por alguna razón ya existiera la malla oficial, se
    #     limpia antes para no duplicar materias.
    for _viejo, _oficial in _REMAP_MALLAS_OFICIAL.items():
        c.execute("SELECT COUNT(*) FROM mallas WHERE programa=?", (_viejo,))
        if c.fetchone()[0] > 0:
            c.execute("DELETE FROM mallas WHERE programa=?", (_oficial,))
            c.execute("UPDATE mallas SET programa=? WHERE programa=?", (_oficial, _viejo))
...
# ------------------------------------------------------------
# VENTANAS DE CARGA DE NOTAS (apertura / cierre)
# ------------------------------------------------------------
# Permiten ABRIR o CERRAR la carga de calificaciones por ámbito:
#   AULA_TALLER, MUNICIPIO, ESTADO, PROGRAMA o GLOBAL (todo el sistema).
# Precedencia al evaluar a un estudiante (gana la primera regla que exista):
#   AULA_TALLER > MUNICIPIO > ESTADO > PROGRAMA > GLOBAL > (por defecto ABIERTA)
# Si no hay ninguna ventana configurada, la carga queda ABIERTA (no rompe el
# uso actual). El Administrador Principal nunca queda bloqueado.

def obtener_ventanas_notas():
    """Devuelve la lista de ventanas de carga de notas como dicts."""
    conn = get_db()
    c = conn.cursor()
    try:
        c.execute("""SELECT id, ambito, valor, estado_ventana, periodo,
                            actualizado_por, fecha_actualizacion
                     FROM ventanas_notas ORDER BY ambito, valor""")
        filas = c.fetchall()
    except sqlite3.OperationalError:
        filas = []
    conn.close()
    cols = ["id", "ambito", "valor", "estado_ventana", "periodo",
            "actualizado_por", "fecha_actualizacion"]
    return [dict(zip(cols, f)) for f in filas]


def guardar_ventana_notas(ambito, valor, estado_ventana, periodo, usuario):
    """Crea o actualiza una ventana de carga de notas."""
    ambito = str(ambito or "").strip().upper()
    valor = str(valor or "").strip()
    estado_ventana = str(estado_ventana or "ABIERTA").strip().upper()
    periodo = str(periodo or "").strip().upper()
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT id FROM ventanas_notas WHERE ambito=? AND valor=?", (ambito, valor))
    if c.fetchone():
        c.execute("""UPDATE ventanas_notas SET estado_ventana=?, periodo=?,
                     actualizado_por=?, fecha_actualizacion=CURRENT_TIMESTAMP
                     WHERE ambito=? AND valor=?""",
                  (estado_ventana, periodo, usuario, ambito, valor))
    else:
        c.execute("""INSERT INTO ventanas_notas (ambito, valor, estado_ventana, periodo, actualizado_por)
                     VALUES (?, ?, ?, ?, ?)""",
                  (ambito, valor, estado_ventana, periodo, usuario))
    conn.commit()
    conn.close()


def eliminar_ventana_notas(vid):
    """Elimina una ventana (vuelve al comportamiento por defecto para ese ámbito)."""
    conn = get_db()
    c = conn.cursor()
    c.execute("DELETE FROM ventanas_notas WHERE id=?", (int(vid),))
    conn.commit()
    conn.close()


def _estado_ventana_por(ambito, valor, ventanas):
    """Busca el estado ('ABIERTA'/'CERRADA') de una ventana concreta. None si no existe."""
    a = str(ambito or "").strip().upper()
    v = str(valor or "").strip().upper()
    for w in ventanas:
        if str(w["ambito"]).strip().upper() == a and str(w["valor"]).strip().upper() == v:
            return str(w["estado_ventana"]).strip().upper()
    return None


def carga_notas_estado(row, ventanas=None):
    """Determina si la carga de notas está ABIERTA para un estudiante (fila de
    expediente). Devuelve (permitida: bool, regla: str, periodo: str).
    Aplica la precedencia AULA_TALLER > MUNICIPIO > ESTADO > PROGRAMA > GLOBAL."""
    if ventanas is None:
        ventanas = obtener_ventanas_notas()
    if not ventanas:
        return True, "Sin restricciones (abierta por defecto)", ""
    ...
    aula = _g(row, "aula_taller", "aula")
    municipio = _g(row, "municipio")
    estado = _g(row, "estado")
    programa = _g(row, "programa")

    for ambito, valor, etiqueta in (
        ("AULA_TALLER", aula, f"Aula taller: {aula}"),
        ("MUNICIPIO", municipio, f"Municipio: {municipio}"),
        ("ESTADO", estado, f"Estado: {estado}"),
        ("PROGRAMA", programa, f"Programa: {programa}"),
        ("GLOBAL", "", "Todo el sistema (global)"),
    ):
        est = _estado_ventana_por(ambito, valor, ventanas)
        if est is not None:
            # período informativo de esa ventana
            per = ""
            for w in ventanas:
                if (str(w["ambito"]).strip().upper() == ambito and
                        str(w["valor"]).strip().upper() == str(valor).strip().upper()):
                    per = str(w.get("periodo") or "")
                    break
            return (est != "CERRADA"), etiqueta, per
    return True, "Sin restricciones (abierta por defecto)", ""
...
with st.sidebar:
    ...
    if rol == "ADMIN_PRINCIPAL":
        menu = st.radio("📍 Módulos", [
            ...
            "🔓 Carga de Notas (Apertura/Cierre)",
            "📊 Ver Calificaciones",
            "📄 Generar Documentos",
...
# ============================================================
    ...
    # Nivel 3 (Administrador Regional): el ESTADO queda FIJO en el estado
    # asignado a la clave. Se fuerza en session_state ANTES de crear el widget
    # porque Streamlit prioriza el valor de session_state sobre el parametro
    # index; si se deja en None el selector muestra "Choose an option" y el
    # estado no queda preseleccionado (bug reportado por el usuario regional).
    _estado_reg_valido = bool(estado_regional) and estado_regional in ESTADOS_MUNICIPIOS
    if _estado_reg_valido:
        st.session_state.reg_estado = estado_regional
    elif "reg_estado" not in st.session_state:
    ...
    with st.expander("🪪 (Opcional) Autocompletar desde una foto o PDF de la cédula", expanded=False):
        ...
        st.caption(
            ...
            "leer la cédula, los nombres, los apellidos y el género (M/F) para ahorrarle tecleo. "
            "**Siempre revise y corrija** los datos antes de registrar. La imagen se "
            "procesa solo en este equipo y **no se guarda**."
        )
        if not _ocr_ok:
            st.info(_ocr_msg)
        else:
                ...
                if not _ced_file:
                    ...
                    if len(_bytes) > 8 * 1024 * 1024:
                        ...
                        if _err:
                            ...
                            # Género (M/F) leído del campo SEXO de la cédula.
                            if _datos.get("sexo") == "F":
                                st.session_state.reg_sexo = "FEMENINO"
                            elif _datos.get("sexo") == "M":
...
# ============================================================
# PÁGINA: CARGA DE NOTAS (APERTURA / CIERRE)
# ============================================================

elif menu == "🔓 Carga de Notas (Apertura/Cierre)":
    mostrar_header("🔓 Carga de Notas — Apertura / Cierre",
                   "Abra o cierre la carga de calificaciones por aula taller, municipio, estado o programa")

    if rol not in ("ADMIN_PRINCIPAL", "ADMIN_AUXILIAR"):
        st.error("🚫 Solo el Administrador Principal o Auxiliar puede abrir/cerrar la carga de notas.")
        st.stop()

    st.info("Aquí decide **quién puede cargar calificaciones y quién no**. "
            "Cuando un ámbito está **CERRADO**, los usuarios no podrán guardar notas de "
            "esos estudiantes hasta que usted lo vuelva a abrir.\n\n"
            "**Orden de prioridad** (gana la regla más específica que exista): "
            "Aula taller ▸ Municipio ▸ Estado ▸ Programa ▸ Global. "
            "Si un estudiante no cae en ninguna regla, su carga queda **ABIERTA**.")

    # ------- Crear / actualizar una ventana -------
    st.markdown("#### ➕ Abrir o cerrar un ámbito")
    tipo_amb = st.radio("¿Qué desea abrir o cerrar?",
                        ["Aula taller", "Municipio", "Estado", "Programa", "Todo el sistema (Global)"],
                        horizontal=True, key="vn_tipo")

    valor_amb = ""
    ambito_cod = "GLOBAL"
    if tipo_amb == "Estado":
        ambito_cod = "ESTADO"
        valor_amb = st.selectbox("Estado", list(ESTADOS_MUNICIPIOS.keys()), key="vn_estado")
    elif tipo_amb == "Municipio":
        ambito_cod = "MUNICIPIO"
        _e = st.selectbox("Estado", list(ESTADOS_MUNICIPIOS.keys()), key="vn_mun_estado")
        valor_amb = st.selectbox("Municipio", ESTADOS_MUNICIPIOS.get(_e, []), key="vn_municipio")
    elif tipo_amb == "Aula taller":
        ambito_cod = "AULA_TALLER"
        _e = st.selectbox("Estado", list(ESTADOS_MUNICIPIOS.keys()), key="vn_aula_estado")
        _m = st.selectbox("Municipio", ESTADOS_MUNICIPIOS.get(_e, []), key="vn_aula_municipio")
        _conn = get_db()
        _c = _conn.cursor()
        _c.execute("SELECT valor FROM listas_editables WHERE tipo_lista='aula_taller' AND categoria_padre=? ORDER BY valor",
                   (f"{_e}|{_m}",))
        _aulas = [r[0] for r in _c.fetchall()]
        _conn.close()
        if _aulas:
            valor_amb = st.selectbox("Aula taller", _aulas, key="vn_aula")
        else:
            valor_amb = st.text_input("Aula taller (escriba el nombre exacto)", key="vn_aula_txt")
    elif tipo_amb == "Programa":
        ambito_cod = "PROGRAMA"
        _todos_prog = [p for lst in TIPOS_PROGRAMA_PROGRAMAS.values() for p in lst]
        valor_amb = st.selectbox("Programa", _todos_prog, key="vn_programa")
    else:
        ambito_cod = "GLOBAL"
        st.caption("La regla **Global** afecta a todos los estudiantes que no tengan una regla más específica.")

    col_a, col_b = st.columns(2)
    with col_a:
        nuevo_estado = st.radio("Estado de la carga", ["ABIERTA", "CERRADA"], horizontal=True, key="vn_estado_ventana")
    with col_b:
        periodo_vn = st.text_input("Período (opcional, ej. 2025-I)", key="vn_periodo")

    if st.button("💾 Guardar regla", type="primary", use_container_width=True, key="vn_guardar"):
        if ambito_cod != "GLOBAL" and not str(valor_amb).strip():
            st.error("⚠️ Seleccione o escriba el valor del ámbito.")
        else:
            guardar_ventana_notas(ambito_cod, valor_amb, nuevo_estado, periodo_vn, usuario)
            _txt = "todo el sistema" if ambito_cod == "GLOBAL" else f"{ambito_cod.replace('_', ' ').title()}: {valor_amb}"
            if nuevo_estado == "ABIERTA":
                st.success(f"✅ Carga **ABIERTA** para {_txt}.")
            else:
                st.warning(f"🔒 Carga **CERRADA** para {_txt}.")
            st.rerun()

    # ------- Reglas activas -------
    st.markdown("---")
    st.markdown("#### 📋 Reglas activas")
    _ventanas = obtener_ventanas_notas()
    if not _ventanas:
        st.info("No hay reglas configuradas. La carga de notas está **ABIERTA** para todos.")
    else:
        _orden = {"AULA_TALLER": 0, "MUNICIPIO": 1, "ESTADO": 2, "PROGRAMA": 3, "GLOBAL": 4}
        for w in sorted(_ventanas, key=lambda x: (_orden.get(str(x["ambito"]).upper(), 9), str(x["valor"]))):
            _amb = str(w["ambito"]).replace("_", " ").title()
            _val = "Todo el sistema" if str(w["ambito"]).upper() == "GLOBAL" else w["valor"]
            _abierta = str(w["estado_ventana"]).strip().upper() != "CERRADA"
            _icono = "🟢" if _abierta else "🔒"
            _txt_est = "ABIERTA" if _abierta else "CERRADA"
            _per = f" · Período {w['periodo']}" if str(w.get("periodo") or "").strip() else ""
            c1, c2 = st.columns([5, 1])
            with c1:
                st.markdown(f"{_icono} **{_amb}** — {_val} · **{_txt_est}**{_per}")
                if str(w.get("actualizado_por") or "").strip():
                    st.caption(f"Actualizado por {w['actualizado_por']} · {w.get('fecha_actualizacion','')}")
            with c2:
                if st.button("🗑️ Quitar", key=f"vn_del_{w['id']}"):
                    eliminar_ventana_notas(w["id"])
                    st.rerun()
...
# ============================================================
    ...
    if df_est.empty:
        ...
        if sel_e:
            ...
            # ¿Está ABIERTA la carga de notas para este estudiante?
            _carga_ok, _carga_regla, _carga_per = carga_notas_estado(row_e)
            # El Administrador Principal nunca queda bloqueado (él administra las ventanas).
            carga_bloqueada = (not _carga_ok) and (rol != "ADMIN_PRINCIPAL")
            if not _carga_ok:
                _per_txt = f" (período {_carga_per})" if str(_carga_per or "").strip() else ""
                if carga_bloqueada:
                    st.error(f"🔒 La carga de notas está **CERRADA** para este estudiante. "
                             f"Regla aplicada — {_carga_regla}{_per_txt}. "
                             f"Solicite al Administrador Principal/Auxiliar que la abra en "
                             f"'🔓 Carga de Notas (Apertura/Cierre)'.")
                else:
                    st.warning(f"🔒 La carga está **CERRADA** para este ámbito ({_carga_regla}{_per_txt}), "
            ...
            if not malla_e:
                ...
                if st.button("💾 Guardar calificaciones", type="primary", use_container_width=True, key="btn_guardar_notas", disabled=carga_bloqueada):
                    notas_dict = {}
                    bloqueadas = []
                    ...
                    try:
                        ...
                        if not col_mat or not col_cal:
                            ...
                            if notas_dict and st.button("💾 Guardar notas del archivo", type="primary",
                                                        use_container_width=True, key=f"btn_xls_{ced_e}", disabled=carga_bloqueada):
                                guardar_notas(ced_e, prog_e, notas_dict)
