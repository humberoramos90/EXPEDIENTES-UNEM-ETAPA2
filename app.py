...
def _parsear_datos_cedula(texto):
    ...
    # las líneas que "parecen un nombre": secuencias de palabras en mayúsculas,
    # sin dígitos, descartando el texto fijo de la tarjeta.
    _STOP = {
    ...
    _tok_re = re.compile(r"^[A-ZÁÉÍÓÚÑÜ]{2,}$")
    candidatos = []
    for l in txt.split("\n"):
        palabras = re.split(r"[^A-Za-zÁÉÍÓÚÑÜáéíóúñü]+", l)
        run, mejor = [], []
        for w in palabras:
            if _tok_re.match(w) and w not in _STOP:
                run.append(w)
            else:
