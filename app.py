...
def init_database():
    ...
    # Tabla de BITACORA: auditoria de acciones sensibles (quien, que y cuando).
...
def aprobar_solicitud(solicitud_id, revisado_por):
    ...
    if campo == CAMPO_ELIMINAR:
        ...
        conn.commit()
        conn.close()
        registrar_bitacora(revisado_por, "APROBAR_ELIMINACION", "expediente", exp_id,
                           "Solicitud de eliminacion aprobada y aplicada")
    ...
    if campo.startswith("NOTA · "):
        ...
        conn.commit()
        conn.close()
        registrar_bitacora(revisado_por, "APROBAR_CAMBIO_NOTA", "expediente", exp_id,
                           f"{campo} -> {valor_nuevo}")
    ...
    conn.commit()
    conn.close()
    registrar_bitacora(revisado_por, "APROBAR_MODIFICACION", "expediente", exp_id,
                       f"{campo} -> {valor_nuevo}")
...
def rechazar_solicitud(solicitud_id, revisado_por):
    ...
    conn.commit()
    conn.close()
    registrar_bitacora(revisado_por, "RECHAZAR_SOLICITUD", "solicitud", solicitud_id,
                       "Solicitud de modificacion/eliminacion rechazada")
...
def registrar_bitacora(usuario, accion, entidad="", entidad_id="", detalle="", rol=""):
    """Escribe UNA linea de auditoria. Nunca interrumpe la operacion principal:
    si algo falla al escribir la bitacora, se ignora en silencio."""
    try:
        conn = get_db()
        c = conn.cursor()
        c.execute("""INSERT INTO bitacora (usuario, rol, accion, entidad, entidad_id, detalle)
                     VALUES (?, ?, ?, ?, ?, ?)""",
                  (str(usuario or "?"), str(rol or ""), str(accion or ""),
                   str(entidad or ""), str(entidad_id or ""), str(detalle or "")))
        conn.commit()
        conn.close()
    except Exception:
        pass
...
def obtener_bitacora(limite=1000, filtro_usuario="", filtro_accion=""):
    """Devuelve las ultimas lineas de la bitacora (mas recientes primero)."""
    conn = get_db()
    c = conn.cursor()
    q = ("SELECT fecha, usuario, rol, accion, entidad, entidad_id, detalle "
         "FROM bitacora")
    cond, params = [], []
    if filtro_usuario:
        cond.append("usuario LIKE ?"); params.append(f"%{filtro_usuario}%")
    if filtro_accion:
        cond.append("accion LIKE ?"); params.append(f"%{filtro_accion}%")
    if cond:
    ...
    try:
        ...
        filas = []
    conn.close()
    cols = ["fecha", "usuario", "rol", "accion", "entidad", "entidad_id", "detalle"]
...
def guardar_notas(cedula, programa, notas_por_materia, usuario=""):
    """Guarda/actualiza las notas del estudiante. notas_por_materia: dict
    {materia: (creditos, nota)} o {materia: (creditos, nota, periodo)}.
    Las notas vacías se ignoran. 'usuario' se registra en la bitácora."""
    conn = get_db()
    c = conn.cursor()
    _guardadas, _borradas = 0, 0
    ...
    conn.commit()
    conn.close()
    if usuario and (_guardadas or _borradas):
        registrar_bitacora(usuario, "GUARDAR_NOTAS", "estudiante", cedula,
                           f"Programa: {programa}. Notas guardadas/actualizadas: "
                           f"{_guardadas}; eliminadas: {_borradas}.")
...
def procesar_carga_masiva_notas(df):
    ...
    for (ced, prog), materias in grupos.items():
        try:
            ...
            guardar_notas(ced, prog, arregladas, usuario)
            ok += len(arregladas)
            ...
            for _, row in df.iterrows():
                ...
                if rol in ("ADMIN_PRINCIPAL", "ADMIN_AUXILIAR"):
                    ...
                    with st.expander("✏️ Editar este expediente (cambio directo)"):
                        with st.form(f"form_edit_{row['id']}"):
                            ...
                            if st.form_submit_button("💾 Guardar cambios", type="primary", use_container_width=True):
                                ...
                                registrar_bitacora(st.session_state.usuario, "EDITAR_EXPEDIENTE",
                                                   "expediente", int(row['id']),
                                                   f"Cédula: {e_cedula.strip().upper()}")
                    ...
                    if st.button("🗑️ Eliminar Expediente", key=f"del_exp_{row['id']}", type="primary", use_container_width=True):
                        if confirmar_del:
                            registrar_bitacora(st.session_state.usuario, "ELIMINAR_EXPEDIENTE",
                                               "expediente", int(row['id']),
                                               f"Cédula: {row.get('cedula','')}")
...
# ============================================================
    ...
    if st.button("💾 Guardar regla", type="primary", use_container_width=True, key="vn_guardar"):
        if ambito_cod != "GLOBAL" and not str(valor_amb).strip():
            ...
            guardar_ventana_notas(ambito_cod, valor_amb, nuevo_estado, periodo_vn, st.session_state.usuario)
        ...
        if sel_e:
            ...
            if not malla_e:
                ...
                if st.button("💾 Guardar calificaciones", type="primary", use_container_width=True, key="btn_guardar_notas", disabled=carga_bloqueada):
                    ...
                    guardar_notas(ced_e, prog_e, notas_dict, st.session_state.usuario)
                    if bloqueadas:
...
# ============================================================
# PÁGINA: BITÁCORA (auditoría) — solo Nivel 1 y Nivel 2
# ============================================================

elif menu == "🗒️ Bitácora":
    mostrar_header("🗒️ Bitácora", "Registro de auditoría: quién hizo qué y cuándo")
    if rol not in ("ADMIN_PRINCIPAL", "ADMIN_AUXILIAR"):
        st.error("🚫 Su nivel de usuario no está autorizado para esta sección. "
                 "La bitácora es exclusiva del Nivel 1 y Nivel 2.")
        st.stop()

    st.caption("Se registran las acciones sensibles: guardado de calificaciones, "
               "edición y eliminación de expedientes y aprobación/rechazo de solicitudes.")

    col_f1, col_f2, col_f3 = st.columns([2, 2, 1])
    with col_f1:
        f_usuario = st.text_input("Filtrar por usuario", key="bit_f_user")
    with col_f2:
        f_accion = st.text_input("Filtrar por acción", key="bit_f_accion")
    with col_f3:
        limite = st.number_input("Máx. filas", min_value=50, max_value=5000,
                                 value=500, step=50, key="bit_limite")

    registros = obtener_bitacora(limite=int(limite),
                                 filtro_usuario=f_usuario.strip(),
                                 filtro_accion=f_accion.strip())

    if not registros:
        st.info("📋 Aún no hay movimientos registrados en la bitácora.")
    else:
        df_bit = pd.DataFrame(registros).rename(columns={
            "fecha": "Fecha", "usuario": "Usuario", "rol": "Rol",
            "accion": "Acción", "entidad": "Entidad",
            "entidad_id": "ID", "detalle": "Detalle",
        })
        st.caption(f"Mostrando {len(df_bit)} movimiento(s).")
        st.dataframe(df_bit, use_container_width=True, hide_index=True)
        st.download_button(
            "⬇️ Descargar bitácora (CSV)",
            data=df_bit.to_csv(index=False).encode("utf-8-sig"),
